...
import {
  STATUS_LABEL,
  STATUS_BADGE,
} from "../contracts/utils/contract.constants";

const BRAND_GREEN = "#3bb582";
const BRAND_BLUE = "#2096d8";
const BRAND_GRADIENT =
  "linear-gradient(135deg, #3bb582 0%, rgba(32,150,216,0.7) 100%)";
    ...
    >
      ...
      <div className="p-5 flex items-start gap-4 flex-1">
        ...
        <div className="min-w-0 flex-1">
          ...
          {!loading && !isEmpty && (
            <p className="text-xs truncate mt-0.5" style={{ color: "#8ab5a3" }}>
              {subtitle}
            </p>
          )}
        </div>
...
function ContractRow({ contract }) {
  ...
  const statusLabel = t(`contracts.status.${contract.status}`, {
    defaultValue: STATUS_LABEL[contract.status] ?? contract.status ?? "—",
  });
  const statusCls =
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0">
          <span
            className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${statusCls}`}
          >
            {statusLabel}
          </span>
          <button
            ...
            style={{ color: "#5A7A6E" }}
            onMouseEnter={(e) => {
              e.currentTarget.style.background = "rgba(59,181,130,0.1)";
...
export default function DashboardPage() {
  ...
  const kpiCards = [
    {
      key: "totalContracts",
      title: t("dashboard.stats.totalContracts"),
      value: totalContracts,
      subtitle: t("dashboard.stats.inSystem"),
      icon: FileText,
      color: "teal",
    },
    {
      key: "totalHouses",
      title: t("dashboard.stats.totalHouses"),
      value: propertyStats.total,
      subtitle: t("dashboard.stats.realEstate"),
      icon: Building2,
      color: "blue",
    },
    {
      ...
      value: totalUsers,
      subtitle: t("dashboard.stats.registered"),
      icon: Users,
      color: "amber",
    },
    {
      key: "rented",
      title: t("dashboard.stats.rentedHouses"),
      value: propertyStats.rented,
      subtitle: t("dashboard.stats.hasContract"),
      icon: Home,
      color: "red",
      isGood: true,
    },
  ...
  return (
    <div className="space-y-5 md:space-y-6">
      <div style={{ paddingTop: 4 }}>
        ...
        <p className="text-sm mt-0.5 capitalize" style={{ color: "#5A7A6E" }}>
          {today}
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5">
        {kpiCards.map(({ key, ...s }) => (
          <KpiCard
            ...
            {...s}
            loading={loading}
            allOnTimeLabel={t("dashboard.stats.allOnTime")}
          />
        ))}
      </div>

      <div
        className="grid grid-cols-1 lg:grid-cols-5 gap-4 md:gap-5"
        style={{ height: 380 }}
      >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-5">
        <div className="flex flex-col" style={{ height: 380 }}>
          <ContractStatusBar
            breakdown={contractStatusBreakdown}
            loading={loading}
          />
        </div>
          ...
          >
            ...
            <div className="flex items-center gap-2">
              {(propertyStats.expiringSoon ?? 0) > 0 && (
                ...
                >
                  <Clock className="w-2.5 h-2.5" />
                  {propertyStats.expiringSoon}{" "}
                  {t("dashboard.recentContracts.expiringSoon")}
                </span>
              )}
              <button
                type="button"
              ...
              >
                {t("dashboard.recentContracts.viewAll")}{" "}
                <ArrowUpRight className="w-3 h-3" />
              </button>
            </div>
          </div>
          ...
          {loading ? (
            <ul className="flex-1">
              {Array.from({ length: 5 }).map((_, i) => (
                <li
                  ...
                  style={{ borderBottom: "1px solid rgba(196,222,213,0.3)" }}
                >
                  <div
                  ...
                  <div className="flex-1 space-y-1.5">
                    ...
                    />
                    <div
                      className="h-2.5 w-1/2 rounded-lg animate-pulse"
                      style={{ background: "#EAF4F0" }}
                    />
                  </div>
                </li>
            ...
            <div className="flex-1 flex flex-col items-center justify-center gap-2 py-10">
              ...
              <p className="text-xs" style={{ color: "#5A7A6E" }}>
                {t("dashboard.recentContracts.empty")}
              </p>
            </div>
          ) : (
