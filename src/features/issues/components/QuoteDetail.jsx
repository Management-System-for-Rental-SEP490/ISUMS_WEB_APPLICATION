...
export default function QuoteDetail({ quote, ticket, houseName, onApproved, onRejected, t }) {
      ...
      <div className="space-y-5">
        <div className="grid grid-cols-[1fr_280px] gap-4">
          ...
          <div className="rounded-2xl p-5" style={{ background: "#FFFFFF", border: "1px solid #C4DED5", borderLeft: "4px solid #3bb582", boxShadow: "0 4px 20px -2px rgba(59,181,130,0.08)" }}>
            <div className="flex items-start justify-between mb-3">
              <div>
                <p className="text-[10px] font-bold uppercase tracking-widest mb-1" style={{ color: "#5A7A6E" }}>{t("issues.quoteRequestId")}</p>
              ...
              <span className="inline-flex items-center gap-1 text-xs font-semibold px-3 py-1 rounded-full" style={{ background: s.bg, color: s.color }}>
                ...
                {t(s.i18nKey, { defaultValue: s.i18nKey })}
              </span>
            </div>
            <div className="space-y-2.5">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-wide" style={{ color: "#5A7A6E" }}>{t("issues.quoteMaintAddress")}</p>
                  <p className="text-sm mt-0.5" style={{ color: "#1E2D28" }}>{houseName ?? t("issues.detailLoading")}</p>
                </div>
              </div>
              <div className="flex items-start gap-2">
                <AlertTriangle className="w-4 h-4 mt-0.5 flex-shrink-0" style={{ color: "#f59e0b" }} />
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-wide" style={{ color: "#5A7A6E" }}>{t("issues.quoteIncidentDesc")}</p>
                  <p className="text-sm mt-0.5 leading-relaxed" style={{ color: "#1E2D28" }}>{ticket.description ?? t("issues.detailNoDesc")}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-2xl p-5 flex flex-col" style={{ background: "#FFFFFF", border: "1px solid #C4DED5", boxShadow: "0 4px 20px -2px rgba(59,181,130,0.08)" }}>
            <p className="text-[10px] font-bold uppercase tracking-widest mb-4" style={{ color: "#5A7A6E" }}>{t("issues.quoteTechSection")}</p>
            {ticket.assignedStaffId ? (
              <>
                <div className="flex items-center gap-3 mb-4">
                  <StaffAvatar name={ticket.staffName} />
                  <div>
                    <p className="text-sm font-bold" style={{ color: "#1E2D28" }}>{ticket.staffName ?? t("issues.staffDefault")}</p>
                ...
                {quote.isTenantFault && (
                  <div className="mt-auto flex items-center gap-1.5 px-3 py-2 rounded-xl" style={{ background: "rgba(217,95,75,0.08)", border: "1px solid rgba(217,95,75,0.2)" }}>
                    ...
                    <p className="text-xs font-semibold" style={{ color: "#D95F4B" }}>{t("issues.quoteTenantFault")}</p>
                  </div>
                )}
              </>
            ) : (
              <div className="flex items-center gap-3" style={{ color: "#5A7A6E" }}>
                ...
                <p className="text-sm">{t("issues.unassignedStaff")}</p>
              </div>
            )}
          </div>
        </div>
        ...
        {images.length > 0 && (
          <div className="rounded-2xl p-5" style={{ background: "#FFFFFF", border: "1px solid #C4DED5", boxShadow: "0 4px 20px -2px rgba(59,181,130,0.08)" }}>
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-[10px] font-bold uppercase tracking-widest" style={{ color: "#5A7A6E" }}>{t("issues.quoteImages")}</p>
        ...
        <div className="rounded-2xl overflow-hidden" style={{ background: "#FFFFFF", border: "1px solid #C4DED5", boxShadow: "0 4px 20px -2px rgba(59,181,130,0.08)" }}>
          <div className="px-6 py-4 flex items-center justify-between" style={{ borderBottom: "1px solid #C4DED5" }}>
            <p className="text-base font-bold" style={{ color: "#1E2D28" }}>{t("issues.quoteSectionTitle")}</p>
            <p className="text-xs" style={{ color: "#5A7A6E" }}>{t("issues.quoteUpdatedAt")} {dayjs(quote.createdAt).format("HH:mm · DD/MM/YYYY")}</p>
          </div>
          <div className="grid grid-cols-[1fr_100px_120px_120px] gap-4 px-6 py-3" style={{ background: "#EAF4F0", borderBottom: "1px solid #C4DED5" }}>
            {[t("issues.quoteColItem"), t("issues.quoteColQty"), t("issues.quoteColUnit"), t("issues.quoteColTotal")].map((h) => (
              <p key={h} className="text-[10px] font-bold uppercase tracking-wider" style={{ color: "#5A7A6E" }}>{h}</p>
            ))}
          </div>
          {(quote.items ?? []).length === 0 && (
            <div className="py-10 text-center text-sm" style={{ color: "#5A7A6E" }}>{t("issues.quoteNoItems")}</div>
          )}
        ...
        <div className="rounded-2xl p-5 flex items-center gap-5" style={{ background: "#FFFFFF", border: "1px solid #C4DED5", boxShadow: "0 4px 20px -2px rgba(59,181,130,0.08)" }}>
          <div className="flex-1 min-w-0 rounded-xl px-5 py-4" style={{ background: "rgba(59,181,130,0.08)", border: "1px solid #C4DED5" }}>
            <p className="text-[10px] font-bold uppercase tracking-widest mb-1" style={{ color: "#3bb582" }}>{t("issues.quoteTotalLabel")}</p>
            <p className="font-heading text-3xl font-black" style={{ color: "#1E2D28" }}>{formatCurrency(quote.totalPrice)}</p>
            <p className="text-xs mt-1" style={{ color: "#5A7A6E" }}>{t("issues.quoteTaxNote")}</p>
          </div>
          {!isApproved && !isRejected ? (
            <div className="flex items-center gap-3 flex-shrink-0">
              ...
              >
                <X className="w-4 h-4" />
                {rejecting ? t("issues.quoteRejectingBtn") : t("issues.quoteRejectBtn")}
              </button>
              <button
              ...
              >
                ...
                {confirming ? t("issues.quoteApprovingBtn") : t("issues.quoteApproveBtn")}
              </button>
            </div>
          ) : isApproved ? (
            <div className="flex items-center gap-2 px-5 py-3 rounded-2xl flex-shrink-0" style={{ background: "rgba(59,181,130,0.10)", border: "1px solid #C4DED5" }}>
              ...
              <p className="text-sm font-bold" style={{ color: "#3bb582" }}>{t("issues.quoteApprovedBadge")}</p>
            </div>
          ) : (
            <div className="flex items-center gap-2 px-5 py-3 rounded-2xl flex-shrink-0" style={{ background: "rgba(217,95,75,0.08)", border: "1px solid rgba(217,95,75,0.3)" }}>
              ...
              <p className="text-sm font-bold" style={{ color: "#D95F4B" }}>{t("issues.quoteRejectedBadge")}</p>
            </div>
          )}
        </div>
        ...
        <div className="rounded-2xl px-5 py-4 flex items-start gap-3" style={{ background: "rgba(32,150,216,0.06)", border: "1px solid rgba(32,150,216,0.2)" }}>
          ...
          <div>
            <p className="text-xs font-bold mb-0.5" style={{ color: "#2096d8" }}>{t("issues.quoteNoteTitle")}</p>
            <p className="text-xs leading-relaxed" style={{ color: "#1a6fa8" }}>
              {t("issues.quoteNoteText")}
            </p>
          </div>
        </div>
      </div>
