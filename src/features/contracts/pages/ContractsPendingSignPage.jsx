  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const all = (Array.isArray(raw) ? raw : (raw?.data ?? [])).map(
          mapContractFromApi,
        );
        // mapContractFromApi converts status to lowercase
        const ready = all.filter((c) => (c.status ?? "") === "READY");
        if (mounted) setContracts(ready);
      } catch (err) {
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-teal-100 to-emerald-200 flex items-center justify-center text-teal-600 border border-teal-300/80">
            <svg
              className="w-5 h-5"
              fill="none"
              viewBox="0 0 24 24"
          ...
          <div>
            <h2 className="text-lg font-bold text-gray-900">Hợp Đồng Cần Ký</h2>
            <p className="text-sm text-gray-500">
              Danh sách hợp đồng đang chờ chủ nhà ký
            </p>
          </div>
        </div>
        ...
        {!loading && contracts.length > 0 && (
          <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-semibold bg-amber-100 text-amber-700 border border-amber-300">
            <span className="w-2 h-2 rounded-full bg-amber-500 animate-pulse" />
            {contracts.length} hợp đồng cần ký
          </span>
        )}
      </div>
      ...
      {/* Loading */}
      {loading && (
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm px-8 py-12 flex flex-col items-center gap-4">
      ...
      {!loading && !error && contracts.length === 0 && (
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm px-8 py-16 flex flex-col items-center gap-3 text-center">
          ...
          <p className="font-semibold text-gray-700">
            Không có hợp đồng nào cần ký
          </p>
          <p className="text-sm text-gray-400">Tất cả hợp đồng đã được xử lý</p>
        </div>
      )}
