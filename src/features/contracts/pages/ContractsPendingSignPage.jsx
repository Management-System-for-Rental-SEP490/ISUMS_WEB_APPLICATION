...
function EmptyState() {
  ...
  return (
    ...
    >
      ...
      <p className="font-semibold" style={{ color: "#1E2D28" }}>{t("contracts.pending.empty")}</p>
      <p className="text-sm" style={{ color: "#5A7A6E" }}>{t("contracts.pending.emptyDesc")}</p>
    </div>
  );
}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr style={{ borderBottom: "1px solid #C4DED5", background: "#EAF4F0" }}>
              ...
              <th className="text-left px-5 py-3.5 text-xs font-semibold uppercase tracking-wide" style={{ color: "#5A7A6E" }}>{t("contracts.pending.contractCode")}</th>
              <th className="text-left px-5 py-3.5 text-xs font-semibold uppercase tracking-wide" style={{ color: "#5A7A6E" }}>{t("contracts.pending.tenant")}</th>
              <th className="text-left px-5 py-3.5 text-xs font-semibold uppercase tracking-wide" style={{ color: "#5A7A6E" }}>{t("contracts.pending.startDate")}</th>
              <th className="text-right px-5 py-3.5 text-xs font-semibold uppercase tracking-wide" style={{ color: "#5A7A6E" }}>{t("contracts.pending.actions")}</th>
            </tr>
          </thead>
          <tbody>
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const msg = err?.message ?? t("feedback.error");
        if (mounted) {
          setError(msg);
          toast.error(msg);
        }
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="font-heading text-3xl font-bold" style={{ color: "#1E2D28" }}>{t("contracts.pending.title")}</h2>
        </div>
        {!loading && contracts.length > 0 && (
          ...
          >
            ...
            {t("contracts.pending.waiting", { count: contracts.length })}
          </span>
        )}
      </div>
      ...
      {loading && (
        <div
          ...
          style={{ background: "#FFFFFF", border: "1px solid #C4DED5" }}
        >
          <LoadingSpinner size="lg" showLabel label={t("contracts.pending.loading")} />
        </div>
      )}
