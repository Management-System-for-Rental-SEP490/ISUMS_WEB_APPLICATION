    ...
    <aside className="w-64 bg-white border-l border-slate-200 flex flex-col overflow-y-auto flex-shrink-0 p-5 gap-3">
      ...
      {!showRejectBox ? (
        ...
        <div className="rounded-xl border border-red-200 bg-red-50 p-3 space-y-2">
          <p className="text-xs font-semibold text-red-700">
            {t("contracts.signing.rejectReason")} <span className="text-red-500">*</span>
          </p>
          <textarea
            rows={3}
            value={rejectReason}
        ...
        <div className="space-y-2">
          {signatureData && (
            <div className="border-2 border-teal-400 rounded-xl overflow-hidden bg-slate-50">
              ...
              <div className="text-center py-1.5 bg-teal-50 border-t border-teal-200">
                <span className="text-[11px] font-semibold text-teal-700">{t("contracts.signing.defaultSignature")}</span>
              </div>
            </div>
          )}
          <button
          ...
          >
            ...
            <span className="text-xs font-medium">
              {signatureData ? t("contracts.signing.createNewSignature") : t("contracts.signing.createSignature")}
            </span>
          </button>
        </div>
      </div>
      ...
      <div className="mt-auto pt-3 border-t border-slate-100">
        <div className="flex items-center justify-center gap-1.5 text-slate-400">
          ...
          <span className="text-[10px] font-medium uppercase tracking-wide">{t("contracts.signing.security")}</span>
        </div>
      </div>
    </aside>
  );
