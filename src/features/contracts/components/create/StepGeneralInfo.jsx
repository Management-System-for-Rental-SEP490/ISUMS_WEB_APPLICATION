...
export default function StepGeneralInfo({ form, update, errors = {} }) {
  ...
  const [showTenantFields, setShowTenantFields] = useState(() => {
    return !!(form?.emailChecked || form?.name || form?.phoneNumber || form?.identityNumber || form?.tenantAddress);
  });
  ...
  const handleCheckEmail = async () => {
    ...
    try {
      ...
      if (user && (user.name || user.identityNumber || user.dateOfIssue || user.fullName)) {
        setEmailLookupUser(user);

        const name = user.name || user.fullName || user.username;
        const identity = user.identityNumber || user.idNumber;
        const dateOfIssue = user.dateOfIssue || user.identityDateOfIssue || "2026-02-21T00:00:00.000Z";
