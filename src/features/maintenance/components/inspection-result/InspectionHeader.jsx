...
const TYPE_CONFIG = {
  CHECK_IN:  { color: "#3bb582", bg: "rgba(59,181,130,0.12)" },
  CHECK_OUT: { color: "#D95F4B", bg: "rgba(217,95,75,0.10)" },
};

function StatusProgressBar({ status, t }) {
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          {showApprove && (
            <button
              ...
              title={!isDone ? t("inspection.header.cannotApprove") : ""}
            >
              <CheckCircle2 className="w-4 h-4" />
              {t("inspection.header.approveBtn")}
            </button>
          )}
        </div>

        <StatusProgressBar status={inspection?.status ?? "CREATED"} t={t} />
      </div>
    </div>
  );
}
