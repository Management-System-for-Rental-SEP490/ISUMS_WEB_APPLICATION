...
export default function Step1JobSelect({
  ...
  const getJobDesc = (job) => {
    ...
    if (jobType === "MAINTENANCE")
      return house
        ? t("schedule.building", { name: house })
        : t("schedule.startDate", { date: formatDateVN(job.periodStartDate) });
    if (jobType === "INSPECTION")
      return house ? t("schedule.building", { name: house }) : "";
  ...
  const getJobDeadline = (job) => {
    ...
    if (jobType === "INSPECTION")
      return daysFromNow(job.scheduledDate ?? job.createdAt, t);
    ...
    <div className="space-y-6">
          ...
          {JOB_TYPES.map((typeItem) => {
            ...
            return (
              ...
              >
                ...
                >
                  {t(
                    `schedule.job${value.charAt(0) + value.slice(1).toLowerCase()}Label`,
                  )}
                </p>
                <p
                ...
                >
                  {t(
                    `schedule.job${value.charAt(0) + value.slice(1).toLowerCase()}Desc`,
                  )}
                </p>
              </button>
            );
      ...
      <div>
        <p className="text-sm font-semibold text-slate-700 mb-1">
          {t("schedule.step1JobLabel")}
        </p>
        <p className="text-xs text-slate-400 mb-3">
          {t("schedule.step1JobDesc")}
        </p>
        ...
        <div className="space-y-2 max-h-64 overflow-y-auto pr-0.5">
          {jobsLoading ? (
            [...Array(4)].map((_, i) => (
              <div
                key={i}
                className="flex items-center gap-0 rounded-xl border border-slate-100 overflow-hidden animate-pulse"
              >
