...
const TYPE_CONFIG = {
  ELECTRICAL: { label: "Điện",          bg: "bg-amber-50",  text: "text-amber-700",  border: "border-amber-200"  },
  PLUMBING:   { label: "Nước",           bg: "bg-blue-50",   text: "text-blue-700",   border: "border-blue-200"   },
  HVAC:       { label: "Điều hòa/HVAC", bg: "bg-teal-50",   text: "text-teal-700",   border: "border-teal-200"   },
  GENERAL:    { label: "Chung",          bg: "bg-slate-50",  text: "text-slate-700",  border: "border-slate-200"  },
};
...
const STATUS_CONFIG = {
  ...
  COMPLETED: { label: "Hoàn thành",     dot: "bg-blue-400",  text: "text-blue-700"  },
};

export default function MaintenancePlanCard({ plan, onViewJobs, onEdit }) {
  const type   = TYPE_CONFIG[plan.type]     ?? TYPE_CONFIG.GENERAL;
  const status = STATUS_CONFIG[plan.status] ?? STATUS_CONFIG.INACTIVE;
